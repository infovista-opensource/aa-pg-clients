...
![Static Badge](https://img.shields.io/badge/Postgres_Client-12-blue) ![Static Badge](https://img.shields.io/badge/alpine-3.18.12-brightgreen)
...
```shell
$ docker run -it --rm --network host piccio/aa-pg-client:12-r0 psql -h 127.0.0.1 -U postgres postgres -c "select version()"
...
```shell
$ docker run --rm --network host -e PGPASSWORD=<PASSWORD> piccio/aa-pg-client:12-r0 psql -h 127.0.0.1 -U postgres postgres -c "select version()"
                                          version                                           
--------------------------------------------------------------------------------------------
 PostgreSQL 14.11 on x86_64-pc-linux-gnu, compiled by gcc (Debian 12.2.0-14) 12.2.0, 64-bit
(1 row)
...
docker run -it --rm --network host piccio/aa-pg-client:12-r0 psql -h 127.0.0.1 -U postgres postgres
...
```
$ docker run -it --rm piccio/aa-pg-client:12-r0  /bin/sh
~ $ id
...
```shell
kubectl -n  empirix-cloud run foo --image=piccio/aa-pg-client:12-r0 --rm -it --command -- /bin/sh
